...
class SecureShieldService
{
    ...
    public function protect(Image $image, array $settings): string
    {
        ...
        // 2. Settings Normalization & User Preference Integration
        $settings['watermark_text'] = $settings['watermark_text'] ?? ($image->user->name ?? 'FocusHub Protected');
        ...
        // 1. Retrieval Logic (Check for existing protected image)
        $fileHash = md5_file(Storage::disk('public')->path($image->path));
        $existing = ProtectedImage::where('hash', $fileHash)
            ->whereJsonContains('settings', $settings)
        ...
        $interventionImage = $this->manager->read(Storage::disk('public')->path($image->path));
        
        // 3. Analysis (Google Vision) for Smart Positioning
        $analysis = $this->analyzeImage($image);

        // 4. Final Application based on Mode
        if ($mode === 'grid') {
        ...
        if (!Storage::disk('public')->exists('protected')) {
            Storage::disk('public')->makeDirectory('protected');
        }

        $interventionImage->toJpeg(90)->save($path);
    ...
    private function applyGlassNeonOverlay(ImageInterface $img, string $text, ?string $logoPath, array $settings = [], array $position = []): void
    {
        ...
        if ($logoPath && Storage::disk('public')->exists($logoPath)) {
            ...
            $img->place($logo, 'top-left', $x + 20, $y + ($patchHeight * 0.2), opacity: 30);
        }
    }
